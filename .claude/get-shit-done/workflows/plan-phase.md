...
@C:/Active Codebase/nirio_features/.claude/get-shit-done/references/ui-brand.md
@C:/Active Codebase/nirio_features/.claude/get-shit-done/references/revision-loop.md
@C:/Active Codebase/nirio_features/.claude/get-shit-done/references/gate-prompts.md
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@C:/Active Codebase/nirio_features/.claude/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
**If found:**
1. Read template: `C:/Active Codebase/nirio_features/.claude/get-shit-done/templates/VALIDATION.md`
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @C:/Active Codebase/nirio_features/.claude/get-shit-done/references/tdd.md to all eligible tasks:
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "C:/Active Codebase/nirio_features/.claude/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
