...
- **Global scope**: `C:/Active Codebase/nirio_features/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Active Codebase/nirio_features/.claude/notes/`)
...
2. Glob `C:/Active Codebase/nirio_features/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Active Codebase/nirio_features/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Active Codebase/nirio_features/.claude/notes/` — works in any directory
