...
> `C:/Active Codebase/nirio_features/.claude/get-shit-done/USER-PROFILE.md` exists). Skip the Read
...
```bash
PROFILE_CONTENT=$(cat "C:/Active Codebase/nirio_features/.claude/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Task()` in parallel:
   ...
   ```
   Task(
     prompt="First, read @C:/Active Codebase/nirio_features/.claude/agents/gsd-advisor-researcher.md for your role and instructions.
